    ...
    try:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                UNIQUE(pseudo, device_id)
            )
        """)
        
        # Migration : ajouter device_id si n'existe pas
        cur.execute("""
            DO $$ 
            BEGIN 
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='users' AND column_name='device_id') THEN
                    ...
                    UPDATE users SET device_id = 'legacy-' || id WHERE device_id IS NULL;
                    ALTER TABLE users ALTER COLUMN device_id SET NOT NULL;
                END IF;
                IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='users' AND column_name='pseudo_lower') THEN
                    ALTER TABLE users DROP COLUMN IF EXISTS pseudo_lower;
                END IF;
                IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='users' AND column_name='secret_key') THEN
                    ALTER TABLE users DROP COLUMN IF EXISTS secret_key;
                END IF;
            END $$;
        """)
        ...
        cur.execute("""
            CREATE INDEX IF NOT EXISTS idx_users_device_pseudo ON users(device_id, pseudo)
        """)
...
def user_login():
    ...
    Identifié par device_id + pseudo.
    - Même device + même pseudo = même utilisateur
    - Nouveau device = nouvel utilisateur
    """
    global USER_TABLES_INITIALIZED
    ...
    try:
        ...
        if not device_id or len(device_id) < 16:
            return jsonify({"status": "error", "message": "device_id invalide"}), 400
        
        conn = get_db_connection()
        cur = conn.cursor()
        
        # Chercher l'utilisateur existant (device_id + pseudo)
        cur.execute(
            "SELECT id, pseudo, device_id, created_at FROM users WHERE device_id = %s AND pseudo = %s",
            (device_id, pseudo)
        )
        user = cur.fetchone()
        ...
        if user:
            ...
            cur.execute(
                "INSERT INTO users (pseudo, device_id) VALUES (%s, %s) RETURNING id, pseudo, device_id, created_at",
                (pseudo, device_id)
            )
            ...
            print(f"👤 Nouvel utilisateur créé: {pseudo} (device: {device_id[:8]}...)")
